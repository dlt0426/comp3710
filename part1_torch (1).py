...
This script:
  ...
  3. Benchmarks three methods across a wide range of signal sizes:
     ...
     The slow NumPy naive DFT is automatically skipped for large N so the
     job does not take forever; FFT vs GPU DFT are compared at every size.
  4. Saves the harmonic-reconstruction plot to PNG.
...
def naive_dft_torch(x):
    ...
    Runs on whatever device x lives on. On the GPU the whole N x N
    ...
    Uses complex128 (double precision) so the result matches numpy's FFT
    to tight tolerance.
    """
    size = x.shape[0]
    dev = x.device
    k = torch.arange(size, device=dev, dtype=torch.float64).reshape(size, 1)
    n = torch.arange(size, device=dev, dtype=torch.float64).reshape(1, size)
    angle = -2.0 * np.pi * k * n / size                   # N x N real angles
    W = torch.exp(1j * angle).to(torch.complex128)        # N x N complex matrix
...
# ----------------------------------------------------------------------
# 4. Correctness check: torch GPU DFT vs numpy FFT
# ----------------------------------------------------------------------
N_check = 512
...
print("torch GPU DFT close to numpy FFT:", np.allclose(gpu_dft, fft_ref, atol=1e-6))
...
def time_numpy_fft(sig_np):
    # average a few runs since a single FFT is extremely fast
    runs = 10
    start = time.time()
    for _ in range(runs):
        np.fft.fft(sig_np)
...
# ----------------------------------------------------------------------
# 6. Three-way benchmark across data sizes
#    NumPy naive is O(N^2) in a python loop, so we only run it up to a
#    cutoff; above that we compare FFT vs GPU DFT only.
# ----------------------------------------------------------------------
sizes = [256, 512, 1024, 2048, 4096, 8192, 16384]
NAIVE_CUTOFF = 2048   # skip the python-loop naive DFT above this N

print("\n--- Timing across data sizes (seconds) ---")
print(f"{'N':>7} | {'NumPy naive':>12} | {'NumPy FFT':>12} | {'Torch GPU DFT':>14}")
print("-" * 58)
...
for N in sizes:
    ...
    if N <= NAIVE_CUTOFF:
        ...
        naive_str = f"{d_naive:>12.6f}"
        methods = {"NumPy naive": d_naive, "NumPy FFT": d_fft, "Torch GPU DFT": d_gpu}
    else:
        d_naive = None
        naive_str = f"{'skipped':>12}"
        methods = {"NumPy FFT": d_fft, "Torch GPU DFT": d_gpu}

    print(f"{N:>7} | {naive_str} | {d_fft:>12.6f} | {d_gpu:>14.6f}")

    order = sorted(methods.items(), key=lambda kv: kv[1])
    print("        fastest -> slowest:",
          " < ".join(f"{name} ({t:.6f}s)" for name, t in order))
